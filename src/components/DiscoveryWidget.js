...
const WidgetContainer = styled.div`
  ...
  border-radius: 15px;
  color: #fff;
  width: ${({ width }) => width || '250px'}; /* Keeping the original size */
  height: ${({ height }) => height || '200px'}; /* Keeping the original size */
...
async function fetchDiscoveryData(timeRange, customStartDate, customEndDate,selectedDoctorId, hospitalId) {
...
const DiscoveryWidget = ({ width, height, timeRange,selectedDoctorId, hospitalId }) => {
  ...
  useEffect(() => {
    async function getDiscoveryData() {
      ...
      const data = await fetchDiscoveryData(timeComponent, customStartDate, customEndDate,selectedDoctorId, hospitalId );
